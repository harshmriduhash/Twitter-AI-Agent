...
if (!process.env.NEXT_PUBLIC_GOOGLE_API_KEY) {
  throw new Error(
    "API Key not found. Please set the NEXT_PUBLIC_GOOGLE_API_KEY environment variable."
  );
}
  ...
  try {
    const response = await agent.invoke({
      messages: new HumanMessage(
        `Post a creative tweet about ${topic} & add emoji to express your sentiments,possiblly add image if you can`
      ),
    });
...
export async function fetchTrendingTopics() {
  try {
    const trendsResponse = await agent.invoke({
      messages: [
        new HumanMessage(
          "Get trending hashtags relevant to technology, AI, DAO, Blockchain, and Crypto etc"
        ),
      ],
...
export async function postTweetUsingTrend(trend: string, topic: string) {
  try {
    const tweet = await agent.invoke({
      messages: [
        new HumanMessage(
          `Post a tweet using the trending hashtag '${trend}' about the topic '${topic}'`
        ),
      ],
    });
    if (tweet?.content) {
    ...
    console.error(
      `Error posting tweet using trend '${trend}' about topic '${topic}':`,
      error
    );
  }
}
...
export async function postPoll(question: string, options: string[]) {
  try {
    const response = await agent.invoke({
      messages: [
        new HumanMessage(
          `generate a provoking & engagement '${question}' and  ${options.join(
            ", "
          )} like a poll  around relevant topics &  post`
        ),
      ],
    });
...
export async function likeATweet(tweetId: string, userId: string) {
  try {
    await agent.invoke({
      messages: [
        new HumanMessage(
          `Like the tweet with id '${tweetId}' and user "${userId}"`
        ),
      ],
    });
...
export async function scrapeCointelegraphHeadlines(url: string) {
  ...
  try {
    const JsonData = await agent.invoke({
      messages: [
        new HumanMessage(
          `scrape data from ${url} using the scrapDataOnlineTool`
        ),
      ],
    });
...
async function createRelevantPostBasedOnSentiment(headlines: string[]) {
  try {
    ...
    const response = await agent.invoke({
      messages: [
        new HumanMessage(
          `using the "${headlines}" create an engaging  post , make sure  you  dont  repeat any headlines you had previously created  `
        ),
      ],
    });
    ...
    if (response && Array.isArray(response?.messages)) {
      ...
      console.log({
        messagge: "cannot  create  a post or  invalid  data structure",
      });
    }
  } catch (error) {
    console.log(error);
...
export async function scrapeAndPostEveryTwoHours() {
  try {
    ...
    const headlines = await scrapeCointelegraphHeadlines(
      "https://www.cointelegraph.com"
    ); // Scrape Cointelegraph for headlines
...
export async function autonomousAgentGoal(goal: string) {
  try {
    const response = await agent.invoke({
      messages: new HumanMessage(
        `this is your  ${goal}. Think of every possible way to achieve it using the available tools or ask for a tool if needed.`
      ),
    });
