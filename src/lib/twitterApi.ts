...
export const accountDetailsTools = tool(
  async () => {
...
export const trendingTopicsTool = tool(
  async () => {
  ...
  },
  {
    ...
    schema: z.object({
      topic: z
        .string()
        .describe(
          "The topic to search for on Twitter, e.g., 'DAO', 'AI agents', 'robotics' etc"
        ),
    }),
  }
);
...
export const scrapDataOnlineTool = tool(
  async ({ url }) => {
    ...
    try {
      ...
      const headlines = await page.evaluate((): string[] => {
        ...
        const elements = document.querySelectorAll(
          "h2,.post-card__title, .article-card__title"
        );
