...
import {
  monitorAndPostRelevantTrends,
  analyzeFollowers,
  postPollIfNeeded,
  searchTweetsUsingTrends,
} from "@/lib/TwitterBot"; // Import your custom functions
...
async function runTweetJob() {
  ...
  if (
    tweetCount.lastReset &&
    isAfter(now, addDays(new Date(tweetCount.lastReset), 1))
  ) {
  ...
  if (tweetCount.count >= MAX_TWEETS_PER_DAY) {
    console.log(
      `Maximum tweets reached for today (${MAX_TWEETS_PER_DAY} tweets).`
    );
    return;
  }
  ...
  const topic =
    randomNumber < 0.5
      ? prioritizedTopics[Math.floor(Math.random() * prioritizedTopics.length)]
...
async function runReplyToMentionsJob() {
  ...
  if (
    lastMentionReplyTime &&
    now.getTime() - lastMentionReplyTime.getTime() < X_MINUTES * 60000
  ) {
    console.log(
      `Skipping mentions, last processed less than ${X_MINUTES} minutes ago.`
    );
    return;
  }
